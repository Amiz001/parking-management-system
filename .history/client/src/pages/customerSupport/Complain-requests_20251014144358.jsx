// ComplaintDashboardWithReply.jsx
...
const ComplaintDashboardWithReply = () => {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      const complaintsWithFlag = res.data.map((c) => ({ ...c, showFullText: false }));
  ...
  const handleReplySend = async () => {
    ...
    try {
      await Axios.put(`http://localhost:5000/api/complaints/reply/${replyPopup.complaint._id}`, {
        reply: replyMessage,
      });
                  ...
                  {filteredComplaints.map((c) => (
                    <tr key={c._id} className="border-b border-gray-700 hover:bg-gray-700">
                      ...
                      <td className="py-3 px-4 max-w-xs">
                        {c.message.length > 50 ? (
                          <span>
                            {c.showFullText ? c.message : `${c.message.slice(0, 50)}... `}
                            <button
                              onClick={() => {
        ...
        {replyPopup.open && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gradient-to-br from-purple-700 via-pink-500 to-orange-400 p-6 rounded-2xl w-full max-w-lg shadow-lg text-white">
              ...
              <p className="mb-4"><strong>Complaint:</strong> {replyPopup.complaint.message}</p>
