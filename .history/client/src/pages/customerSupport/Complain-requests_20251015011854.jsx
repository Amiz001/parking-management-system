  ...
  const exportToPDF = () => {
    ...
    AutoTable(doc, {
      ...
      body: filteredComplaints.map(c => [
        c.name || "-", c.email || "-", c.subject || "-", c.description || "-", c.status || "-", c.createdAt ? new Date(c.createdAt).toLocaleDateString() : "-"
      ]),
      theme: "striped",
              ...
              <tbody>
                {filteredComplaints.map((complaint )=> (
                  <tr key={complaint._id} className="border-b border-gray-700 hover:bg-gray-700">
                    ...
                    <td className="py-2 px-3 max-w-xs">{complaint.description || "-"}</td>
                    ...
                    <td className="py-3 px-4">{new Date(refund.createdAt).toLocaleDateString()}</td>
                      <td className="py-3 px-4 flex gap-2">
                        <button onClick={() => setReplyPopup({ open: true, complaintId: complaint._id })} className="px-3 py-1 bg-blue-600 rounded-md hover:bg-blue-700 text-white">Reply</button>
