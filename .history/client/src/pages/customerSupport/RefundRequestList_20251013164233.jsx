...
const RefundDashboardPlanetBlue = () => {
  ...
  return (
    <div className={`flex h-auto bg-gradient-to-b from-blue-900 via-blue-800 to-blue-950 text-white ${lightMode ? "light bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 text-black" : ""}`}>
      {/* Sidebar */}
      <div className="w-64 h-screen bg-blue-950 p-6 flex flex-col">
        ...
        <div className="mb-8">
          ...
          <nav className="space-y-2">
            {sidebarItems.map((item, i) => (
              <a key={i} onClick={() => navigate(item.link)} className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${item.label === "Refund requests" ? "bg-gradient-to-l from-blue-500 to-blue-700 text-white" : "text-gray-300 hover:bg-blue-800"}`}>
                <item.icon size={20} />
        ...
        <div className="mb-8">
          ...
          <nav className="space-y-2">
            {userItems.map((item, i) => (
              <a key={i} onClick={() => navigate(item.link)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-300 hover:bg-blue-800 transition-colors">
                <item.icon size={20} />
                <span>{item.label}</span>
              </a>
            ))}
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-blue-950 p-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
                <input type="text" id="searchInput" placeholder="Search refund..." onChange={(e) => filterRefunds(e.target.value)} className="pl-10 pr-4 py-2 bg-blue-900 border border-blue-800 rounded-lg text-white focus:ring-2 focus:ring-blue-400" />
              </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredRefunds.map((refund) => (
                    <tr key={refund._id} className="border-b border-blue-800 hover:bg-blue-800">
                      ...
                      <td className="py-3 px-4 flex gap-2">
                        <button onClick={() => openReplyPopup(refund)} className="px-3 py-1 bg-blue-600 rounded-md hover:bg-blue-700 text-white">Reply</button>
        ...
        {replyPopup.open && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gradient-to-br from-blue-700 via-blue-500 to-blue-600 p-6 rounded-2xl w-full max-w-lg shadow-lg text-white">
              <h2 className="text-2xl font-bold mb-4">Reply to Refund</h2>
              <p className="mb-2"><strong>Customer Name:</strong> {replyPopup.refund.name}</p>
              <p className="mb-2"><strong>Email:</strong> {replyPopup.refund.email}</p>
              <p className="mb-4"><strong>Refund Reason:</strong> {replyPopup.refund.reason}</p>
              ...
              <textarea
                ...
                className="w-full h-24 p-3 mb-3 rounded-md bg-blue-800 text-white"
              />
              ...
              <textarea
                ...
                className="w-full h-20 p-3 mb-3 rounded-md bg-blue-800 text-white"
              />

              <div className="flex justify-end gap-3 mt-2">
                <button
                  onClick={() => setReplyPopup({ open: false, refund: null })}
                  className="px-4 py-2 bg-blue-900 rounded-md hover:bg-blue-700"
                >
                  Cancel
                </button>
                <button
...
export default RefundDashboardPlanetBlue;
