...
const ComplaintForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ===================== USER ID VALIDATION =====================
    if (!userId) {
      setError("⚠️ User ID is required.");
      return;
    }

    // ===================== NAME VALIDATION =====================
    if (!name) { setError("⚠️ Name is required."); return; }
    if (name.length < 3) { setError("⚠️ Name must be at least 3 characters."); return; }
    if (!validateAlpha(name)) { setError("⚠️ Name must contain only alphabets."); return; }
    if (name.length > 30) { setError("⚠️ Name cannot exceed 30 characters."); return; }

    // ===================== EMAIL VALIDATION =====================
    if (!email) { setError("⚠️ Email is required."); return; }
    if (!validateEmailFormat(email)) { setError("⚠️ Invalid email format."); return; }

    // ===================== SUBJECT VALIDATION =====================
    if (!subject) { setError("⚠️ Subject is required."); return; }
    if (!validateAlphanumeric(subject)) { setError("⚠️ Subject must contain only alphanumeric characters."); return; }
    if (subject.length > 50) { setError("⚠️ Subject cannot exceed 50 characters."); return; }

    // ===================== COMPLAINT VALIDATION =====================
    if (!complaint) { setError("⚠️ Complaint is required."); return; }
    if (complaint.length < 10) { setError("⚠️ Complaint must be at least 10 characters."); return; }
      ...
      <div className={`w-full max-w-2xl rounded-2xl p-8 shadow-xl backdrop-blur-lg ${lightMode ? "bg-white/90 border border-gray-200" : "bg-gradient-to-b from-[#151821]/90 to-[#242938]/90 border border-gray-800"}`}>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className={`block mb-2 text-sm font-medium ${lightMode ? "text-gray-700" : "text-gray-300"}`}>User ID</label>
              <input type="text" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="Enter your User ID" className={`w-full px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 ${lightMode ? "bg-white border border-gray-200 text-black" : "bg-[#151821] border border-gray-700 text-white"}`} />
            </div>
