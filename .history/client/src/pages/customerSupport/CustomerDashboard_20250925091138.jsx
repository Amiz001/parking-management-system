  ...
  const renderSection = () => {
    switch (activeSection) {
          ...
          >
            ...
            <div className="space-y-2">
              {feedbackData.map((f) => (
                <div
                  key={f.id}
                  className="p-3 rounded-lg bg-indigo-600 text-white flex justify-between items-center"
                >
                  <span>{f.message}</span>
      ...
      case "complaints":
        ...
        const complaintChartData = Object.keys(complaintSummary).map((key) => ({
          name: key,
          value: complaintSummary[key],
        }));
          ...
          >
            ...
            <div className="space-y-2">
              {complaintData.map((c) => (
                <div
                  key={c.id}
                  className="p-3 rounded-lg bg-red-600 text-white flex justify-between items-center"
                >
                  <span>{c.message}</span>
                  <span>{c.status}</span>
                </div>
      ...
      case "tickets":
        ...
        const ticketChartData = Object.keys(ticketSummary).map((key) => ({
          name: key,
          value: ticketSummary[key],
        }));
          ...
          >
            ...
            <div className="space-y-2">
              {ticketData.map((t) => (
                <div
                  key={t.id}
                  className="p-3 rounded-lg bg-green-600 text-white flex justify-between items-center"
                >
                  <span>{t.issue}</span>
                  <span>{t.status}</span>
                </div>
      ...
      case "refunds":
        ...
        const refundChartData = Object.keys(refundSummary).map((key) => ({
          name: key,
          value: refundSummary[key],
        }));
          ...
          >
            ...
            <div className="space-y-2">
              {refundData.map((r) => (
                <div
                  key={r.id}
                  className="p-3 rounded-lg bg-yellow-500 text-black flex justify-between items-center"
                >
          ...
          >
            ...
            <div className="space-y-2">
              {notificationData.map((n) => (
                <div
                  key={n.id}
                  className="p-3 rounded-lg bg-blue-500 text-white flex flex-col gap-1"
                >
    ...
    >
      ...
      <aside
        className={`w-64 p-6 flex flex-col ${
          lightMode ? "bg-white shadow-lg" : "bg-gray-900"
        }`}
      >
