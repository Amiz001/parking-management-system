              ...
              <tbody>
                {filteredFeedbacks.map((f) => (
                  <FeedbackRow
                    key={f._id}
                    feedback={f}
                    onReply={() => setReplyPopup({ open: true, feedbackId: f._id })}
...
// Subcomponent for feedback rows
const FeedbackRow = ({ feedback, onReply, onDelete }) => {
  const [expanded, setExpanded] = useState(false);

  return (
    <tr className="border-b border-gray-700 hover:bg-gray-700">
      <td className="py-2 px-3">{feedback.name || "-"}</td>
      <td className="py-2 px-3">{feedback.email || "-"}</td>
      <td className="py-2 px-3">{feedback.category || "-"}</td>
      <td className="py-2 px-3 max-w-xs">
        {feedback.feedback && feedback.feedback.length > 30 ? (
          <>
            {expanded ? feedback.feedback : `${feedback.feedback.slice(0, 30)}...`}
            <button
      ...
      <td className="py-2 px-3">{feedback.rating || "-"}</td>
