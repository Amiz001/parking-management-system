...
const FeedbackDashboard = () => {
  ...
  const exportToPDF = () => {
    ...
    AutoTable(doc, {
      startY: 20,
      head: [["Name", "Email", "Category", "Feedback", "Rating", "Submitted At"]],
      ...
      <div className="w-64 h-screen bg-[#151821] p-6 flex flex-col">
        ...
        <div className="mb-8">
          ...
          <nav className="space-y-2">
            ...
            <a onClick={() => navigate("/customersupport/feedback-requests")} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-gradient-to-l from-blue-500 to-indigo-600 text-white">Feedbacks</a>
          </nav>
        </div>
                ...
                <tbody>
                  {filteredFeedbacks.map(f => (
                    <tr key={f._id} className="border-b border-gray-700 hover:bg-gray-700">
                      ...
                      <td className="py-3 px-4 max-w-xs">{f.feedback || f.message || "-"}</td>
                      <td className="py-3 px-4">{f.rating || "-"}</td>
                      <td className="py-3 px-4">{f.createdAt ? new Date(f.createdAt).toLocaleDateString() : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
