...
const TicketDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nameRegex.test(name)) return setError("⚠️ Name can only contain letters and spaces.");

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!email) return setError("⚠️ Email is required.");
    if (!emailRegex.test(email)) return setError("⚠️ Enter a valid email address.");
    if (email.length > 50) return setError("⚠️ Email cannot exceed 50 characters.");
    ...
    if (subject.length > 100) return setError("⚠️ Subject cannot exceed 100 characters.");

    if (!description) return setError("⚠️ Description is required.");
    if (description.length < 10) return setError("⚠️ Description must be at least 10 characters.");
    if (description.length > 500) return setError("⚠️ Description cannot exceed 500 characters.");

    if (!["Low", "Medium", "High"].includes(priority)) return setError("⚠️ Please select a valid priority.");
