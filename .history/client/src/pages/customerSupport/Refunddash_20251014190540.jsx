...
const CustomerSupportPage = () => {
  ...
  // ✅ Updated "Ticket Request" path
  const sidebarItems = [
    { icon: Ticket, label: "Ticket Request", link: "/customersupport/teckset" },
    { icon: AlertCircle, label: "Complain Request", link: "/customersupport/comreqy" },
    { icon: MessageSquare, label: "Feedback Request", link: "/customersupport/feedback-requests" },
  ];

  const bottomItems = [
    { icon: Settings, link: "/customersupport/setting", label: "Settings" },
  ...
  return (
    <div
      className={`flex h-auto bg-gray-950 text-white ${
        lightMode ? "light bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 text-black" : ""
      }`}
    >
      {/* Sidebar */}
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-gray-950 p-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => filterRequests(document.getElementById("searchInput").value)}
          ...
          <div className="flex items-center gap-4">
            ...
            <Settings
              size={20}
              onClick={() => navigate("/customersupport/setting")}
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="bg-gradient-to-b from-[#151821] to-[#242938] rounded-xl p-6 overflow-x-auto">
                ...
                {filteredRequests.map((request) => (
                  <tr key={request._id} className="border-b border-gray-700 hover:bg-gray-700">
                          ...
                          <button
                            onClick={() =>
                              setFilteredRequests((prev) =>
                                prev.map((r) =>
                                  r._id === request._id
                                    ? { ...r, showFullReason: !r.showFullReason }
                                    : r
                                )
                              )
                            }
                    ...
                    <td className="py-3 px-4 flex gap-2">
                      <button
                        onClick={() => openReplyPopup(request)}
        ...
        {replyPopup.open && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gradient-to-br from-purple-700 via-pink-500 to-orange-400 p-6 rounded-2xl w-full max-w-lg shadow-lg text-white">
              ...
              <p className="mb-2">
                <strong>Customer Name:</strong> {replyPopup.request.name}
              </p>
              <p className="mb-2">
                <strong>Email:</strong> {replyPopup.request.email}
              </p>
              <p className="mb-4">
                <strong>Reason:</strong> {replyPopup.request.reason}
              </p>
