...
const RefundDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { icon: SquareParking, label: "Slot Management", link: "/admin/slots" },
    { icon: HeartHandshake, label: "Membership plans", link: "/admin/membership" },
    { icon: BanknoteArrowDown, label: "Refund requests", link: "/admin/refund-requests" },
    { icon: Megaphone, label: "Notifications", link: "/admin/notifications" },
  ];

  const userItems = [
    { icon: Users, label: "User Management", active: true, link: "/admin/users" },
  ...
  // Test function for debugging export
  const testExport = () => {
  ...
  // Make testExport available globally for debugging
          ...
          <nav className="space-y-2">
            {bottomItems.map((item, index) => (
              <a
                ...
                className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-300 hover:bg-gray-700 cursor-pointer transition-colors"
              >
                <item.icon size={20} />
                <span>{item.label}</span>
              </a>
