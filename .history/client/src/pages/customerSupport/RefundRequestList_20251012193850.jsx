...
const RefundDashboard = () => {
  ...
  // Initial fetch + refresh if navigated with state
  useEffect(() => {
    fetchRefunds();
        ...
        <main className="flex-1 p-6 overflow-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-400 light:text-gray-600">Manage all refund requests here</p>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={exportToExcel} className="flex gap-2 items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">📊 Export Excel</button>
              <button onClick={exportToPDF} className="flex gap-2 items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">📄 Export PDF</button>
            </div>
          </div>
                  ...
                  {filteredRefunds.map((refund, index) => (
                    <tr key={refund._id} className="border-b border-gray-200 hover:bg-gray-700">
                      ...
                      <td className="py-4 px-4 text-gray-300 relative">
                        ...
                        {menuIndex === index && (
                          <div ref={menuRef} className="absolute right-0 w-28 bg-gray-900 text-white border border-gray-700 rounded-md shadow-lg z-20">
