...
import { 
  Search, Sun, Moon, Settings, LogOut, Home, Ticket, AlertCircle, MessageSquare, Bell 
} from "lucide-react";
              ...
              <tbody>
                {filteredComplaints.map((complaint) => (
                  <tr key={complaint._id} className="border-b border-gray-700 hover:bg-gray-700">
                    ...
                    <td className="py-2 px-3 max-w-xs">{complaint.complaint || "-"}</td>
                    <td className="py-2 px-3">{complaint.createdAt ? new Date(complaint.createdAt).toLocaleDateString() : "-"}</td>
                    <td className="py-2 px-3 flex gap-2">
                      <button
                        onClick={() => console.log("Reply clicked", complaint._id)}
