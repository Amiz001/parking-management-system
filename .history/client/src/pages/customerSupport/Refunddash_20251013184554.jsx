...
const CustomerSupportPage = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await Axios.get("http://localhost:5000/api/refund"); // Replace with customer support API if needed
  ...
  const handleDelete = async (requestId) => {
    try {
      await Axios.delete(`http://localhost:5000/api/refund/${requestId}`); // Replace with proper API
    ...
    <div className={`flex h-auto bg-gray-950 text-white ${lightMode ? "light bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 text-black" : ""}`}>
      ...
      <div className="w-64 h-screen bg-[#151821] p-6 flex flex-col">
        <div className="flex items-center gap-3 mb-8">
          ...
          <span className="text- font-semibold">Customer Support</span>
        </div>
