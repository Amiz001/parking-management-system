...
const RefundRequestList = () => {
  ...
  const fetchRefunds = async () => {
    try {
      const refunds = await Axios.get("http://localhost:5000/refunds");

      if (refunds.data.message === "Not Found") {
        setRefunds([]);
        return;
      }
  ...
  const filteredRefunds = (searchTerm) => {
    ...
    const filtered = refunds.filter((u) => {////
  ...
  const handleDelete = async (refundId) => {
    try {
      const response = await Axios.delete(
        `http://localhost:5000/refunds/${refundId}`
      );
  ...
  const exportToPDF = () => {
    if (!filteredRefunds || filteredRefunds.length === 0) {
      toast.error("No Refund to export");
      return;
    }

    const doc = new jsPDF();
    doc.text("Parkbay - Refunds Report", 14, 10);

    AutoTable(doc, {////
      startY: 20,
      head: [["Name", "Email", "Role", "Membership", "Joined on", "Status"]],
      body: filteredRefunds.map((u) => [
        u.name,
        u.email,
        u.role,
        u.membership,
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => filteredRefunds(e.target.value)}
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-400 light:text-gray-600">
                Here is user details of the system
              </p>
            </div>
            <div className="flex items-center gap-4">
              {/*<select
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium">{refund.name}</span>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-gray-300 light:text-gray-600">
                        {refund.email}
