...
const RefundDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { icon: BanknoteArrowDown, label: "Refund requests", link: "/admin/refundrequestlist" },
          ...
          <nav className="space-y-2">
            {userItems.map((item, index) => (
              <a
                key={index}
                onClick={() => item.link && navigate(item.link)}
                className="flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors text-gray-300 hover:bg-gray-700"
              >
                <item.icon size={20} />
                <span>{item.label}</span>
              </a>
