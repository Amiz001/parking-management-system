...
import { 
  Search, Sun, Moon, Settings, LogOut, Home, Ticket, AlertCircle, MessageSquare, Bell, Trash2, Mail 
} from "lucide-react";
  ...
  const exportToPDF = () => {
    ...
    AutoTable(doc, {
      ...
      body: filteredTickets.map(t => [
        t.name || "-", t.email || "-", t.subject || "-", t.description || "-", t.priority || "-", t.createdAt ? new Date(t.createdAt).toLocaleDateString() : "-"
      ]),
      theme: "striped",
           ...
           <div className="flex items-center gap-4">
             <div className="flex items-center gap-2">
               <div className="relative">
                 ...
                 <input type="text" id="searchInput" placeholder="Search ticket..." onChange={(e) => filterRefunds(e.target.value)} className="pl-10 pr-4 py-2 bg-[#151821] border border-gray-900 rounded-lg text-white focus:ring-2 focus:ring-blue-500" />
               </div>
               <button onClick={() => filterTickets(document.getElementById("searchInput").value)} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
              ...
              <tbody>
                {filteredTickets.map(t => (
                  <tr key={t._id} className="border-b border-gray-700 hover:bg-gray-700">
                    ...
                    <td className="py-2 px-3">{t.createdAt ? new Date(t.createdAt).toLocaleDateString() : "-"}</td>
