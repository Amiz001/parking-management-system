...
import { 
  Search, Settings, Bell, Sun, Moon, 
  ChartColumnBig, SquareParking, HeartHandshake, 
  BanknoteArrowDown, Megaphone, Users, Car, 
  LogOut, Mail, Trash2 
} from "lucide-react";
...
const RefundDashboard = () => {
  ...
  const fetchRefunds = async () => {
    try {
      const res = await Axios.get("http://localhost:5000/refunds");
  ...
  const handleDelete = async (refundId) => {
    try {
      await Axios.delete(`http://localhost:5000/refunds/${refundId}`);
  ...
  const handleReplySend = async () => {
    ...
    try {
      await Axios.put(`http://localhost:5000/refunds/reply/${replyPopup.refundId}`, {
        reply: replyMessage,
        additionalNotes
      });
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-400">
                    ...
                    <th className="py-3 px-4 text-left">Transection ID</th>
                  ...
                  {filteredRefunds.map((refund) => (
                    <tr key={refund._id} className="border-b border-gray-700 hover:bg-gray-700">
                      ...
                      <td className="py-3 px-4 max-w-xs">
                        {refund.reason.length > 50 ? (
                          <span>
                            ...
                            <button onClick={() => setFilteredRefunds(prev =>
                              prev.map(r => r._id === refund._id ? { ...r, showFullReason: !r.showFullReason } : r)
                      ...
                      <td className="py-3 px-4 flex gap-2">
                        <button onClick={() => setReplyPopup({ open: true, refundId: refund._id })} className="flex items-center gap-1 px-2 py-1 bg-blue-600 rounded hover:bg-blue-700 text-white text-sm">
                          <Mail size={16} /> Reply
                        </button>
                        <button onClick={() => handleDelete(refund._id)} className="flex items-center gap-1 px-2 py-1 bg-red-600 rounded hover:bg-red-700 text-white text-sm">
        ...
        {replyPopup.open && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gradient-to-br from-blue-700 via-blue-500 to-blue-600 p-6 rounded-3xl w-full max-w-md relative z-10">
              ...
              <textarea value={replyMessage} onChange={e => setReplyMessage(e.target.value)} placeholder="Enter your reply..." className="w-full h-28 p-3 rounded-xl bg-blue-800 text-white mb-4 focus:outline-none focus:ring-2 focus:ring-blue-400" />
              <textarea value={additionalNotes} onChange={e => setAdditionalNotes(e.target.value)} placeholder="Additional notes (optional)..." className="w-full h-20 p-3 rounded-xl bg-blue-800 text-white mb-4 focus:outline-none focus:ring-2 focus:ring-blue-400" />
              <div className="flex justify-between">
                <button onClick={() => { setReplyPopup({ open: false, refundId: null }); setReplyMessage(""); setAdditionalNotes(""); }} className="px-6 py-2 rounded-xl bg-blue-900 hover:bg-blue-700 transition-colors">Cancel</button>
                <button onClick={handleReplySend} className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 transition-colors">Send Reply</button>
              </div>
            </div>
          </div>
        )}
