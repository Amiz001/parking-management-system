        ...
        <main className="flex-1 p-6 overflow-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-400">Manage all refund requests here</p>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={exportToExcel} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"> Export Excel</button>
              <button onClick={exportToPDF} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"> Export PDF</button>
            </div>
          </div>
                  ...
                  {filteredRefunds.map((refund, index) => (
                    <tr key={refund._id} className="border-b border-gray-700 hover:bg-gray-700">
                      ...
                      <td className="py-3 px-4 relative">
                        ...
                        {menuIndex === index && (
                          <div ref={menuRef} className="absolute right-[2%] bg-gray-900 text-white border border-gray-700 rounded-md shadow-lg z-20">
                            <ul className="divide-y divide-gray-700">
                              <li className="px-4 py-2 hover:bg-blue-600 cursor-pointer" onClick={() => setReplyPopup({ open: true, refundId: refund._id })}>💬 Reply</li>
                              <li className="px-4 py-2 hover:bg-red-600 cursor-pointer" onClick={() => handleDelete(refund._id)}><Trash size={16} /> Delete</li>
                            </ul>
                          </div>
                        )}
                      </td>
